...
G=[GCR(:,efree),GL(:,list2)];                  %%% Matriz general G=[GCR GL]
%%%%------------------------------------
% 
...
sys1=[D*GCR(:,efree), GL(:,list2)];
sys2=[GCR(:,efree)'; GL(:,list2)'*inv(D)];


lambda_max1(j)=abs(max(eigs(sys1)));
lambda_max2(j)=abs(max(eigs(sys2)));
...
media_NCondACR=mean(n_conACR)
media_NCondAL=mean(n_conAL)
media_NCondABD=mean(n_conABD)

varianza_NCondACR=var(n_conACR)
varianza_NCondAL=var(n_conAL)
varianza_NCondABD=var(n_conABD)
