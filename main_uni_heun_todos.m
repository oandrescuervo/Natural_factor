...
% Euler1
M_t1=M0-dt*M0*(A1-A0)*M0;
% Euler2 (atrás)
M_t11=M1+dt*M1*(A1-A0)*M1;
% Promedio Euler
M_t2=(M_t1+M_t11)/2;

% Heun
M_t3=M0-dt/2*(M0*(A1-A0)*M0+M_t1*(A1-A0)*M_t1);
% Heun2 (atrás)
M_t31=M1+dt/2*(M1*(A1-A0)*M1+M_t11*(A1-A0)*M_t11);
% "Promedio Heun"
M_t4=(M_t3+M_t31)/2;
...
%Nivel 0
ncond_M0(j)=cond(M0*ABD);
ncond_prom(j)=cond(((M0+M1)/2)*ABD);

%Nivel 1
ncond_euler(j)=cond(M_t1*ABD);
ncond_prom_euler(j)=cond(M_t2*ABD);

%Nivel 2
ncond_heun(j)=cond(M_t3*ABD);
ncond_prom_heun(j)=cond(M_t4*ABD);
